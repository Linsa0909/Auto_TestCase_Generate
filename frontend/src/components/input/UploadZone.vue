  ...
  <div>
    <div
      :class="[
        ...
        dragging ? 'border-zinc-400 bg-zinc-100' : 'border-zinc-300 hover:border-zinc-400 hover:bg-zinc-50',
      ]"
    ...
    >
      <UploadCloud
        :class="['w-5 h-5 transition-all', dragging ? 'text-zinc-900' : 'text-zinc-400 group-hover:text-zinc-600']"
        :stroke-width="1.5"
      />
    ...
    <div v-if="files.length" class="mt-3 flex flex-col gap-1.5">
      <div
        ...
        class="flex items-center gap-3 px-4 py-2.5 rounded-lg bg-zinc-50 border border-zinc-200 hover:border-zinc-300 text-sm transition-colors group/file"
      >
        <FileText class="w-4 h-4 text-zinc-400 shrink-0" :stroke-width="1.5" />
        <span class="flex-1 text-zinc-700 truncate">{{ file.name }}</span>
        <span class="text-xs text-zinc-400">{{ formatSize(file.size) }}</span>
        <button
          @click="$emit('remove', i)"
          class="w-5 h-5 flex items-center justify-center rounded text-zinc-400 hover:text-red-500 hover:bg-red-50 transition-colors opacity-0 group-hover/file:opacity-100"
        >
