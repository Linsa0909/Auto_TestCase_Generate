      ...
      <div class="flex items-center justify-between pb-4 border-b border-zinc-800/60">
        ...
        <div v-if="result" class="flex items-center gap-2">
          ...
          <button
            ...
            class="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border border-indigo-600 text-indigo-300 hover:border-indigo-400 hover:text-indigo-200 transition-colors active:scale-[0.98]"
          >
...
import { Sparkles, Download, Upload, RefreshCw } from 'lucide-vue-next'
...
async function pushCasesToDevops() {
  ...
  pushingCases.value = true
  pushCasesMsg.value = '推送中...'
  try {
